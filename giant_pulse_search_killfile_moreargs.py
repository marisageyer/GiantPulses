...
parser = argparse.ArgumentParser(usage='describe usage here',
                                     description='description of script here.',epilog='Copyright (C) 2019 by Marisa Geyer and Jones Chilufya')
parser.add_argument('-dir', dest='directory_name', metavar='<directory_name>', help='specify directory where single pulse archives directories are')
parser.add_argument('-mask', dest='mask_file', metavar='<mask_file>', help='specify mask file where RFI mask')
...
for snr_file in snr_data_files:
    ...
    if snr >= snr_max:
       # print "High SNR in", snr
        ...
        if (phase_max > ph_lim_Aug and phase_max < ph_lim_Aug2):
